...
<?php if (!empty($banner)) : ?>
	...
	<section    data-aos-delay="600"  data-aos="fade-up"
			 class="relative bg-contain  bg-no-repeat bg-right-bottom   h-[60vh] md:h-[80vh] flex items-center  dots__overlay-left" style="background-image: url('<?php echo esc_url($banner['background_img']['url']); ?>');">
		<div class="container mx-auto text-left text-black p-12  lg:px-24 2xl:px-16   mb-20">
			...
			<?php if (!empty($banner['header'])) : ?>
				<h1 class="text-5xl md:text-6xl font-bold mb-6"><?php echo wp_kses_post($banner['header']); ?></h1>
			<?php endif; ?>
