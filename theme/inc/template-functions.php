...
function footer_logo()
{
	...
	if (have_rows('footer_content', 'option')) :
		while (have_rows('footer_content', 'option')) : the_row();
			...
			// Generate the HTML output for the logo
			$logo = '<img width="300" height="300" src="' . $logo_url . '" class="footer-logo"  alt="'.$site_name.'"  decoding="async">';
...
{
	if ($post_type === 'page' || $post_type === 'inwestycje'  || $post_type === 'post'   ) {
		$use_block_editor = false;
		//remove_post_type_support('page', 'editor');
		//remove_post_type_support('inwestycje', 'editor');
...
function my_submit_form_tag_handler( $tag ) : string {
	return '<button type="submit" class="btn btn--primary gap-4 text-lg">
