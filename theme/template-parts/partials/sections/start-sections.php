        ...
        <?php foreach ($sections as $index => $section) : ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:gap-24 items-center mb-10 relative <?php echo $bg_class?>" >
                ...
                <?php if (!empty($section['section']['img']['url'])) : ?>
                    <div data-aos-delay="200" data-aos="<?php echo $is_even ? 'fade-right' : 'fade-left'; ?>" class="<?php echo $img_class; ?>">
                        <img src="<?php echo esc_url($section['section']['img']['url']); ?>" alt="" class="w-full h-auto shadow-lg rounded-lg">
                    </div>
                <?php endif; ?>

                <!-- Content -->
                <div data-aos-delay="300" data-aos="<?php echo $is_even ? 'fade-left' : 'fade-right'; ?>" class="<?php echo $content_class; ?>">
                    <?php if (!empty($section['section']['header'])) : ?>
                        <h2 class="font-barlow text-3xl lg:text-4xl font-bold text-black mb-12 relative  <?php echo $arrow_class; ?>"><?php echo wp_kses_post($section['section']['header']); ?></h2>
                    <?php endif; ?>
                    ...
                    <?php if (!empty($section['section']['list']['list_item'])) : ?>
                        <ul class="ml-8 start__list list-inside my-8">
                            <?php foreach ($section['section']['list']['list_item'] as $list_item) : ?>
								<li class="mb-3"><?php echo strip_tags($list_item['content'],['</br>', '<strong>']); ?></li>
                            <?php endforeach; ?>
                        </ul>
                    <?php endif; ?>

                    <?php if (!empty($section['section']['button']['content']) && !empty($section['section']['button']['url']) && $section['section']['button']['url'] !== '#') : ?>
                        <div class="text-left my-8">
                            <a href="<?php echo esc_url($section['section']['button']['url']); ?>" class="inline-block text-white rounded">
                                <button class="btn btn--primary"> <?php echo esc_html($section['section']['button']['content']); ?></button>
                            </a>
                        </div>
                    <?php endif; ?>
                </div>
