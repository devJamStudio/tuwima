...
<div data-aos-delay="100"  data-aos="fade-up"  class="flex  gap-2 lg:gap-[3rem] my-20 text-black   text-black font-barlow  max-w-[65rem] justify-between items-center   mx-auto">
	...
	<div class="content flex  max-h-fit justify-center w-full  min-w-[16rem] lg:min-w-[20rem] max-w-[30rem]  items-center  <?php echo $content_order; ?>">
		<div class="flex  max-h-fit  gap-4">
			<div class="flex flex-col max-h-fit <?php echo $inner_order; ?>">
		<h3 class="text-lg lg:text-3xl text-black font-barlow font-bold mb-4"><?php echo $block['header']; ?></h3>

		<p class="text-md text-black" ><?php echo $block['content']; ?></p>
			</div>
			<div class="flex items-center <?php echo $circle_order; ?>">
			<span class="bg-primary w-16 h-16 lg:w-20 lg:h-20  text-lg text-white md:text-2xl lg:text-4xl font-bold rounded-[50rem] aspect-square flex items-center justify-center  <?php echo $arrow_order; ?>"><?php echo $block['index']; ?></span>
			</div>
		</div>
	</div>
</div>
